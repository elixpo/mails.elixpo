...
export default function TemplateComposer({ templateId }: { templateId?: string }) {
    ...
    useEffect(() => {
        ...
        fetch("/api/products")
            .then((r) => r.json())
            .then((d: any) => {
                if (!alive || !d?.ok || !Array.isArray(d.products) || d.products.length === 0)
                    return;
                const p =
                    d.products.find(
        ...
        fetch(`/api/templates/${templateId}`)
            ...
            .then((d: any) => {
                ...
                if (d?.ok) {
                    ...
                    setAttachments(
                        Array.isArray(t.attachments) ? (t.attachments as Attachment[]) : [],
                    );
            ...
            {templateId && (
                <TemplateTestDialog
                    ...
                    getContentHtml={async () =>
                        apiRef.current ? await apiRef.current.getHTML() : ""
                    }
                    bgColor={bgColor}
                />
            )}
