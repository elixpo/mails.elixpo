...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    if (recipients.length === 0) {
        return NextResponse.json({ ok: false, error: "Enter at least one valid recipient." }, { status: 400 });
    }
    ...
    try {
        ...
        // Optional sender override → set it on a shallow template clone so the
        // pipeline's sender-resolution chain picks it first.
        let effectiveTemplate = template;
        if (typeof body?.senderId === "string" && body.senderId) {
            const sender = await getSender(db, session.tenantId, body.senderId);
            if (!sender) return NextResponse.json({ ok: false, error: "Unknown sender." }, { status: 400 });
            effectiveTemplate = { ...template, sender_id: sender.id };
        }
        ...
        if (typeof body?.aliasId === "string" && body.aliasId) {
            ...
            const alias = senderId ? await getAlias(db, session.tenantId, senderId, body.aliasId) : null;
